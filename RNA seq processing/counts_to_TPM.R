...
countToTpm <- function(rawCount, GSEID) {    
	...
	geneLengthMap = read.table(file="../Annotation/mm10_gene_length_kb.txt", sep = "\t", header = FALSE,stringsAsFactors = F)
	geneCol = 2
	commonGenes = intersect(rawCount[,1], geneLengthMap[,1])
	
	if(length(commonGenes) == 0) {
		geneLengthMap = read.delim("../Annotation/hg38_gene_length_kb.txt", sep = "\t", header = FALSE)
		commonGenes = intersect(rawCount[,1], geneLengthMap[,1])
		geneCol = 2
	}
	...
	outFile = paste("../Data_generated/", paste(GSEID, "TPM.tsv", sep = "_"), sep = "")
...
rnaToMA = function(TPMVal, GSEID){
	MA = (0.57 +(TPMVal* 0.37 ))
	
	output_matrix = cbind(rownames(TPMVal), MA[,1:ncol(MA)])
	colnames(output_matrix) = c('Gene_symbol', colnames(TPMVal))
	outFile = paste("../Data_generated/", paste(GSEID, "gene-exp.txt", sep = "_"), sep = "")
	if(!file.exists(outFile)) file.create(outFile)
	write.table(output_matrix, file = outFile, sep = '\t', quote = F, row.names = F)
